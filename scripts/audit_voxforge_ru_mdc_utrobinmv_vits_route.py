"""Publish the write-once exact-route audit for VoxForge RU UtrobinTTS VITS."""
...
from kds.data.utrobinmv_vits import UtrobinmvVitsError, load_utrobinmv_vits
from kds.eval.generator_route_gate import GeneratorRouteGateError, audit_generator_route_exposure

_HISTORICAL_MODEL_IDENTIFIERS = frozenset({"tts_ru_free_hf_vits_low_multispeaker"})
...
def _legacy_vits_evidence(exposure_rows: dict[str, list[ManifestRow]]) -> dict[str, object]:
    matches: list[str] = []
    routes: Counter[tuple[str, str, str]] = Counter()
    for path, rows in sorted(exposure_rows.items()):
        for row in rows:
            if row.label != "spoof" or "vits" not in (
                row.generator_family + " " + row.generator_name
            ).casefold():
                continue
    ...
    return {
        ...
        "interpretation": (
            "Historical manifests contain generic VITS-family rows, so the candidate cannot claim "
            "architecture-family novelty. Their evidence has no UtrobinTTS revision, safetensors "
            "hash, or fixed woman-0 route, and therefore does not prove exact-route reuse."
        ),
    }


def _unversioned_identifier_evidence(
    ...
    return {
        ...
        "interpretation": (
            "The historical model-name identifier matches this candidate, but its rows lack an "
            "immutable upstream revision and weights hash. Exact route novelty is therefore not "
            "provable and the candidate must be rejected rather than relabelled as unseen."
        ),
    }
...
def main() -> int:
    ...
    try:
        ...
        if len(lock.models) != 1:
            raise ResearchTtsError("UtrobinTTS route audit requires exactly one model lock entry.")
        model = lock.models[0]
        runtime = load_utrobinmv_vits(arguments.model_root, model)
        exposure_rows: dict[str, list[ManifestRow]] = {}
        ...
        gate = audit_generator_route_exposure(
            ...
            fixed_voice_aliases=("utrobinmv_vits:woman:0",),
        )
        if gate["exact_route_overlap_rows"] != 0:
            raise GeneratorRouteGateError("Unexpected non-zero UtrobinTTS exact-route overlap.")
        unversioned = _unversioned_identifier_evidence(exposure_rows)
        if unversioned["rows"] != 0:
            raise GeneratorRouteGateError(
                "Historical manifests contain the candidate UtrobinTTS model identifier without "
                "a pinned revision/hash; exact-route novelty cannot be proven."
            )
        payload = {
            "schema_version": 1,
            "protocol_id": "voxforge-ru-mdc-utrobinmv-vits-female-exact-route-audit-v1",
            ...
            "runtime_policy": {
                "fixed_voice_id": "utrobinmv_vits:woman:0",
            ...
            "route_gate": gate,
            "legacy_vits_evidence": _legacy_vits_evidence(exposure_rows),
            "unversioned_candidate_identifier_evidence": unversioned,
            "claims": {

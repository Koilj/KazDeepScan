"""Publish a write-once Silero V5.5 RU/eugene exact-route audit before synthesis."""
...
def _legacy_silero_evidence(exposure_rows: dict[str, list[ManifestRow]]) -> dict[str, object]:
    matches: list[str] = []
    routes: Counter[tuple[str, str, str]] = Counter()
    for path, rows in sorted(exposure_rows.items()):
        for row in rows:
            if row.label != "spoof" or "silero" not in row.generator_name.casefold():
                continue
    ...
    return {
        ...
        "interpretation": (
            "Historical manifests include Silero V4 routes and generic unpinned Silero labels. "
            "They prevent a vendor-family or architecture-family novelty claim, but do not prove "
            "reuse of the pinned V5.5 package/runtime/eugene route."
        ),
    }
...
def main() -> int:
    ...
    try:
        ...
        if len(lock.models) != 1:
            raise ResearchTtsError("Silero V5.5 route audit requires exactly one model lock entry.")
        model = lock.models[0]
        runtime = load_silero_v5_5_runtime(model)
        verified = verify_research_tts_model_lock(arguments.model_root, lock)[model.model_id]
        inspect_silero_v5_5_package(verified[runtime.package_path])
        ...
        gate = audit_generator_route_exposure(
            ...
            fixed_voice_aliases=(SILERO_V5_5_FIXED_SPEAKER,),
        )
        payload = {
            "schema_version": 1,
            "protocol_id": "silero-v5-5-ru-eugene-exact-route-audit-v1",
            ...
            "runtime_policy": {
                "fixed_voice_id": runtime.fixed_speaker,
            ...
            "route_gate": gate,
            "legacy_silero_evidence": _legacy_silero_evidence(exposure_rows),
            "claims": {
                "exact_generator_route_absent_from_historical_manifests": True,
                "architecture_family_novelty": False,
                "vendor_family_independence": False,

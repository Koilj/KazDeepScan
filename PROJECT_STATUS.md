...
- Silero V5.5 RU / fixed `eugene` принят только как новый exact route для будущего RU
  ...
  raw 48 kHz WAV на каждый text. Technical QA, review и detector inference ещё не начаты.
...
- Fixed V5.5 literal-text gate без lexical rewrite проверил ровно эти `6 211` metadata survivors:
  ...
  один raw WAV per bound text. Synthetic technical QA, acoustic review и inference не начаты.

...
> **Актуальное ограничение scope (10 августа 2026):** проект не собирает и не записывает
> голоса людей, не ищет участников и не создаёт voice clones по reference audio. Данные берутся
> из готовых источников после rights/provenance review либо синтезируются локально text-to-speech
> моделями со встроенными публичными voices. Старые consented-collection идеи ниже заменены этим
> правилом.
...
| Commercial-clean | архивный будущий контур, не текущая цель | только готовые данные и TTS-выходы с отдельно проверенным разрешением на коммерческое ML-обучение |
| Hold-out/OOD | финальная честная проверка, **никогда не train и не выбор гиперпараметров** | новые готовые TTS/dataset sources, независимые model families, тексты и каналы |
...
### 5.4. Стратегия данных без собственной записи

- Bona-fide RU/KK/mixed брать только из готовых releases с проверенными лицензией, archive
  checksum, provenance и доступными speaker/device groups.
- Если speaker или device metadata отсутствуют, сохранять `unknown` и не выдавать split за
  speaker/device-disjoint.
- Synthetic clips создавать локально только из текста и встроенных голосов закреплённых TTS
  checkpoints. Reference audio, voice cloning и имитация конкретного человека запрещены.
- Для каждого нового generator family фиксировать source revision, веса, SHA-256, voice/control
  ID, seed, runtime, лицензию, raw/ready manifests и полный rejection report.
- Независимый test получать из нового готового source или новой архитектурной generator family;
  не переиспользовать раскрытые final assets и не считать новым family другой voice того же
  checkpoint.
...
| F5-TTS | да, после quality-test | проверить без reference audio | research OOD | только встроенные/public model voices; официальные pretrained weights CC BY-NC 4.0 |
| XTTS-v2 | да | проверить без reference audio | research OOD | только non-cloning режим после проверки лицензии; Coqui Public Model License non-commercial |

Модели, которым для нужного режима требуется reference audio конкретного человека, в этот
проект не добавлять.
...
| Утечка голосов | не собирать собственные голоса; минимизировать локальные готовые assets, исключить их из Git и соблюдать source terms |

#!/usr/bin/env python3
"""Validate and execute the one-time XLS-R+SLS v2 Stage-C research evaluation."""
...
def _cuda_device() -> torch.device:
    if not torch.cuda.is_available():
        raise RuntimeError("Stage-C XLS-R inference requires an available CUDA device.")
    device = torch.device("cuda")
    if not torch.cuda.is_bf16_supported():
        raise RuntimeError("Stage-C XLS-R inference requires CUDA BF16 support.")
    return device
...
def _require_unused_outputs(plan: XlsrStageCPlan) -> None:
    existing = [
        str(path) for path in (plan.outputs.execution_lock, plan.outputs.report) if path.exists()
    ]
    if existing:
        raise ValueError(
            "Refusing to run this one-time Stage-C plan because output already exists: "
            + ", ".join(existing)
            + "."
        )
...
def _load_stage_b_state(plan: XlsrStageCPlan) -> dict[str, Tensor]:
...
def _validate_embedded_encoder_and_head(
    checkpoint: Mapping[str, object], plan: XlsrStageCPlan
) -> None:
...
def _build_model(
    plan: XlsrStageCPlan, state: Mapping[str, Tensor], device: torch.device
) -> XlsrSlsClassifier:
...
def _layer_report(
    ...
    for source, indices in sorted(source_indices.items()):
        ...
        source_expected = source_labels >= 0.5
        source_predictions = source_probabilities >= 0.5
        source_correct = int((source_predictions == source_expected).sum())
        label_counts = Counter(rows[index].label for index in indices)
        source_metrics[source] = {
            "label_counts": dict(sorted(label_counts.items())),
            "class_accuracy": _metric(source_correct, len(indices)),
            "mean_calibrated_spoof_probability": float(source_probabilities.mean()),
        }
...
def _validate_assets(inputs: ValidatedStageCInputs, audio_root: Path) -> int:
    rows = [
        *inputs.calibration,
        *(row for language_rows in inputs.final_by_language.values() for row in language_rows),
    ]
...
def main(argv: list[str] | None = None) -> int:
    ...
    plan = load_xlsr_stage_c_plan(arguments.plan)
    ledger = load_license_ledger(plan.license_ledger.path)
    inputs = validate_xlsr_stage_c_inputs(plan, ledger)
    ...
    preflight: dict[str, object] = {
        ...
        "run_plan": stage_c_plan_record(plan),
        "assets_validated": asset_count,
        "role_rows": {
            "calibration": len(inputs.calibration),
            **{language: len(rows) for language, rows in inputs.final_by_language.items()},
        },
    ...
    for language in ("ru", "kk", "mixed"):
        rows = inputs.final_by_language[language]
        ...
        if not torch.equal(labels, expected_labels):
            raise RuntimeError(
                f"Dataset labels differ from manifest for Stage-C language {language}."
            )
        layer_reports[language] = _layer_report(rows, logits, calibration_report.temperature)
    ...
    report: dict[str, object] = {
        **preflight,
        ...
        "limitations": [
            ...
            "Exact Stage-C assets were not previously inferred in configured project roles, but "
            "the suite is not source- or speaker-independent.",
            "The fixed Male2 voice alias overlaps 312 historical Piper rows through a different "
            "runtime route.",
...
if __name__ == "__main__":
    ...
    except (XlsrStageCPlanError, OSError, RuntimeError, ValueError) as error:

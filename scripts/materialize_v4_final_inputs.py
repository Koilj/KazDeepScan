"""Run or review-finalize the one-shot v4 final-input materialization contract."""
...
def main() -> int:
    ...
    materialize = modes.add_parser("materialize")
    materialize.add_argument("--plan", type=Path, required=True)
    materialize.add_argument("--project-root", type=Path, default=Path("."))
    materialize.add_argument("--data-root", type=Path, default=Path("data"))
    materialize.add_argument("--common-voice-archive", type=Path, required=True)
    materialize.add_argument("--fleurs-release-root", type=Path, required=True)
    materialize.add_argument("--workers", type=int, default=4)
    ...
    preflight.add_argument("--common-voice-archive", type=Path, required=True)
    preflight.add_argument("--fleurs-release-root", type=Path, required=True)
    preflight.add_argument("--created-at", required=True)
    ...
    except (OSError, RuntimeError, ValueError, V4FinalMaterializationError) as error:

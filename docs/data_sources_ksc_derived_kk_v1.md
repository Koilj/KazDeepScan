...
Самостоятельный набор создаётся только так:

1. Собрать bona-fide записи с документированным согласием на обучение, синтетические
   производные и нужный режим использования; сохранить PII и подписанные согласия вне Git,
   а в репозитории — pseudonymous consent registry.
2. До записи закрепить сценарии: язык, диалекты, устройства, помещения, шум, длину и текст.
   Разделить людей и тексты по train/dev/test **до** генерации, чтобы один голос или текст не
   оказался в нескольких ролях.
3. Для каждой независимой TTS/VC family записать exact model revision, checkpoint hash,
